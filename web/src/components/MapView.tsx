...
export function MapView() {
  ...
  useEffect(() => {
    ...
    syncMarker(map, toMarkerRef, mode === "route" ? to : null, "to", profile);
  }, [from, to, profile, mode]);

  useEffect(() => {
    const map = mapRef.current;
    if (!map || !mapReadyRef.current) return;
    setRouteData(map, route, profile);
    if (route?.found && route.geometry.length > 1) {
      fitToCoords(map, route.geometry, 80);
    }
  }, [route, profile]);
  ...
  function applyAll() {
    ...
    syncMarker(map, toMarkerRef, s.mode === "route" ? s.to : null, "to", s.profile);
    setRouteData(map, s.route, s.profile);
    setIsoData(map, s.isochrone);
  }
...
function setRouteData(map: MlMap, route: import("@/lib/types").RouteResponse | null, profile: string) {
  ...
  if (!src) return;
  if (!route || !route.found || route.geometry.length < 2) {
...
function syncMarker(
  ...
  // Always tear-down + rebuild so a profile change re-colors the pin cleanly.
  ref.current?.remove();
